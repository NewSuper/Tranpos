    ...
    public static byte[] pixToEscNvBitImageCmd(byte[] src, int width, int height) {
        ...
        for(int i = 0; i < width; ++i) {
            ...
            for(int j = 0; j < height / 8; ++j) {
                data[4 + j + i * height / 8] = (byte)(p0[src[i + var7]] + p1[src[i + var7 + width]] + p2[src[i + var7 + 2 * width]] + p3[src[i + var7 + 3 * width]] + p4[src[i + var7 + 4 * width]] + p5[src[i + var7 + 5 * width]] + p6[src[i + var7 + 6 * width]] + src[i + var7 + 7 * width]);
                var7 += 8 * width;
            }
        }
