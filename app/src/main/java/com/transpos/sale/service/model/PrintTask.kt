package com.transpos.sale.service.model
...
import com.transpos.sale.service.UsbPhoneService
import com.transpos.sale.ui.scan.manger.OrderPayManger
import com.transpos.sale.utils.ActivityCollector
import com.transpos.tools.usbPrint.EscCommand
import com.transpos.tools.usbPrint.UsbPrintUtils
import org.apache.commons.lang3.ArrayUtils
import java.io.Serializable
import java.util.*

class PrintTask(val context: Context,val tradeNo : String?) : Runnable {
    ...
    override fun run() {
        tradeNo?.run {
            ...
            var payInfo : OrderPay? = null
            if(payInfos.isNotEmpty()){
                payInfo = payInfos[0]
            }
            val byteArray =
                ArrayList<ByteArray>()
            ...
            for (item in orderItemList) {
                ...
                Log.e("debug","name=$name   amount=$amount   price=$price   buy=$buy_price   tradeNo=${item.tradeNo}")
                byteOrder.addAll(
                    UsbPrintUtils.PrintOrderList88mm(
                        name,
                        amount,
                        price,
                        buy_price
...
//            esc.addText("                抹零金额：0" + "\n", "GB2312")
            ...
            payInfo?.let {
                ...
                esc.addText("                    ${e.payName}：${order.receivableAmount}" + "\n", "GB2312")
            }

            esc.addText("------------------------------------------------\n", "GB2312")
            if(order.isMember == 1){
                esc.addText("本次积分:${order.addPoint.toInt()}             剩余积分：${order.aftPoint.toInt()}\n","GB2312")
                esc.addText("会员号：${order.memberNo}\n","GB2312")
