...
//        mTvUnreadCount.setPadding(padding, 0, padding, 0);
    ...
    public void setSelected(boolean selected) {
        ...
        Log.d(TAG, "mTvTitle: "+ mTvTitle.getText().toString()+"  selected:"+selected);
//        if (mTabPosition == 1) return;  //赚车赚房选项
        if (mTvTitle.getText().toString().length() == 0) return;

        String tab = mTvTitle.getText().toString();
        if(mContext.getString(R.string.home).equals(tab)){
            mIcon.setImageResource(selected?R.mipmap.ic_nav_home:R.mipmap.ic_nav_home_normal);
        }else if(mContext.getString(R.string.movie).equals(tab)){
            mIcon.setImageResource(selected?R.mipmap.ic_movie_vip:R.mipmap.ic_movie_vip_unselect);
        }else if(mContext.getString(R.string.task).equals(tab)){
            mIcon.setImageResource(selected?R.mipmap.ic_nav_task:R.mipmap.ic_nav_task_normal);
        }else if(mContext.getString(R.string.mine).equals(tab)){
            mIcon.setImageResource(selected?R.mipmap.ic_nav_my:R.mipmap.ic_nav_my_normal);
        }
        mTvTitle.setTextColor(selected ? ContextCompat.getColor(mContext, R.color.tab_selected) : ContextCompat.getColor(mContext, R.color.tab_un_select));
    }
    ...
    public void showLoadingByPos(int imageResource, final int position){
