...
public class VideoChannelManagerActivity extends BaseActivity<ChannelManagerPresenterImpl, ActivityChannelManagerBinding> implements View.OnClickListener, ChannelManagerPresenter.ChannelManagerView {
    ...
    private String defaultChannelNames = "热门,影视,汽车,音乐,体育,旅游";
    ...
    public void initView() {
        ...
        mViewBinding.headLayout.toolbarTitle.setText("我的视频");
...
//        loadOtherChannel();
    ...
    private void setData(List<ChannelEntity> otherChannels) {
        showList = Paper.book().read(PagerCons.VIDEO_CHANNEL_CHCHE);
        ...
        final ChannelAdapter adapter = new ChannelAdapter(this, helper, showList, otherChannels, 3);
        ...
        adapter.setEditingListener(new ChannelAdapter.EditedListener() {
            ...
            public void updateChannel(List<ChannelEntity> list) {
                Paper.book().write(PagerCons.VIDEO_CHANNEL_CHCHE, list);
                showList = list;
            }
        });
    }
    ...
    protected void onDestroy() {
        ...
        if (notifyUpdate) {
            if (showList.size() < defaultList.size()) {
                //如果小时默认个数则使用默认的
                Paper.book().write(PagerCons.VIDEO_CHANNEL_CHCHE, defaultList);
            }
            EventBus.getDefault().post(new UpdateVideoChannelEvent());
        }
    }
