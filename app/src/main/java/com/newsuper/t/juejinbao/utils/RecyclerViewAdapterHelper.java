...
public abstract class RecyclerViewAdapterHelper<T> extends RecyclerView.Adapter<RecyclerView
    ...
    public void refreshRecyclerView(List<T> _list) {
        mList.addAll(0, _list);
    ...
    public void reloadRecyclerView(List<T> _list, boolean isClear) {
        ...
        int prePosition = mList.size();
        mList.addAll(_list);
        int position = mList.size();
        if (isClear) {
            notifyDataSetChanged();
        } else {
//            notifyItemRangeChanged(prePosition,position);
            try {
                notifyItemRangeInserted(prePosition, position);
            } catch (Exception e) {
