...
public class PageFactory {
    ...
    private List<BookChapterEntity.Chapter> chaptersList;
    ...
    public PageFactory(Context context, int width, int height, int fontSize, String bookId,
                       List<BookChapterEntity.Chapter> chaptersList) {
        mContext = context;
        mWidth = width;
        mHeight = height;
        mFontSize = fontSize;
        ...
        mTitlePaint.setColor(ContextCompat.getColor(JJBApplication.getContext(), R.color.chapter_title_day));
    ...
    public synchronized void onDraw(Canvas canvas) {
        ...
        if (mLines.size() > 0) {
            int y = (int) (marginHeight + (mLineSpace << 1)+ mContext.getResources().getDimension(R.dimen.ws20dp));
            // 绘制背景
            if (mBookPageBg != null) {
            ...
            // 绘制标题
            canvas.drawText(chaptersList.get(currentChapter - 1).getName(), marginWidth, y, mTitlePaint);
            y += mLineSpace;
