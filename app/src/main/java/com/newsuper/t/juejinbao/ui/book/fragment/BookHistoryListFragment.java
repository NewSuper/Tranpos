...
import com.newsuper.t.databinding.FragmentBookHistoryListBinding;
 ...
 * 最近阅读
 */
public class BookHistoryListFragment extends BaseFragment<BookshelfPresenterImpl, FragmentBookHistoryListBinding> implements BookshelfPresenterImpl.MvpView,
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        return inflater.inflate(R.layout.fragment_book_history_list, container, false);
    }
    ...
    public void initView() {
        ...
        mViewBinding.refresh.setOnRefreshListener(new OnRefreshListener() {
            ...
            public void onRefresh(RefreshLayout refreshlayout) {
                mPresenter.getMyNovelRecordList(mActivity);
            }
        });
    }
    ...
    public void click(View view) {
        ...
        switch (view.getId()){
            case R.id.iv_delete:
                new AlertDialog.Builder(mActivity).setMessage("是否将该小说从最近阅读列表移除?").setCancelable(false)
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        mPresenter.delFromMyNovelRecord(mActivity, bookShelfList.get(position).getNovel().getId());
                        dialog.dismiss();
                    }
                }).create().show();
                break;
            case R.id.tv_read:
                BookReadingActivity.intentMe(mActivity, bookShelfList.get(position).getNovel().getId(), "",-1,0);
                break;
            case R.id.rl_item:
