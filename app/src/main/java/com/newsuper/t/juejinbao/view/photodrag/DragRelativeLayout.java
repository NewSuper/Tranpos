...
public class DragRelativeLayout extends RelativeLayout {
    ...
    // 设置接口
    private com.newsuper.t.juejinbao.view.photodrag.OnDragListener mListener;
    ...
    public boolean dispatchTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction() & event.getActionMasked()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                float dcx = x - downX;
                float dcy = y - downY;
                if (Math.abs(dcx) > 8 && Math.abs(dcy) > 8) {
                    //通过距离差判断方向
                    int orientation = getOrientation(dcx, dcy);
                    switch (orientation) {
                        case 'r':
                            action = "右";
                            break;
                    ...
                    Log.e("TAG", "dispatchTouchEvent:华东方向======》》》》》》 " + "向" + action + "滑动");
                }
    ...
    // 设置回调
    public DragRelativeLayout setOnoDragListener(com.newsuper.t.juejinbao.view.photodrag.OnDragListener listener) {
        mListener = (com.newsuper.t.juejinbao.view.photodrag.OnDragListener) listener;
        return this;
    }
