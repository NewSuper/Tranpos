...
public abstract class PageLoader {
    ...
    // 当前章节列表
    protected List<BookChapterEntity.Chapter> mChapterList;
    // 书本对象
    ...
    private Subscription mPreLoadDisp;
    ...
    private void initData() {
        ...
        // 初始化参数
        mMarginWidth = (int) ScreenUtils.dpToPx(DEFAULT_MARGIN_WIDTH);
        mMarginHeight = (int) ScreenUtils.dpToPx(DEFAULT_MARGIN_HEIGHT);
        // 配置文字有关的参数
    ...
    public void setNightMode(boolean nightMode) {
        ...
        if (isNightMode) {
            mBatteryPaint.setColor(mContext.getResources().getColor(R.color.chapter_content_night));
            setPageStyle(PageStyle.NIGHT);
        } else {
            mBatteryPaint.setColor(mContext.getResources().getColor(R.color.chapter_content_day));
     ...
     * @return
     */
    public List<BookChapterEntity.Chapter> getChapterCategory() {
        return mChapterList;
    }
    ...
    public void saveRecord() {
        ...
        mBookRecord.setBookId(mCollBook.getNovel().getId());
        ...
        //存储到数据库
        Paper.book().write(mCollBook.getNovel().getId(),mBookRecord);
    }
    ...
    private void prepareBook() {
        mBookRecord = Paper.book().read(mCollBook.getNovel().getId());
    ...
    private List<TxtPage> loadPageList(int chapterPos) throws Exception {
        // 获取章节
        BookChapterEntity.Chapter chapter = mChapterList.get(chapterPos);
        // 判断章节是否存在
     ...
     * @return
     */
    protected abstract BufferedReader getChapterReader(BookChapterEntity.Chapter chapter) throws Exception;
     ...
     * @return
     */
    protected abstract boolean hasChapterData(BookChapterEntity.Chapter chapter);
    ...
    private void drawBackground(Bitmap bitmap, boolean isUpdate) {
        ...
        /*int tipMarginHeight = (int) ScreenUtils.dpToPx(3);
        if (!isUpdate) {
            *//****绘制背景****//*
            canvas.drawColor(mBgColor);
      ...
      /*  int visibleRight = mDisplayWidth - mMarginWidth;
        ...
        int polarHeight = (int) ScreenUtils.dpToPx(6);
        int polarWidth = (int) ScreenUtils.dpToPx(2);
        int border = 1;
        int innerMargin = 1;
        ...
        Rect polar = new Rect(polarLeft, polarTop, visibleRight,
                (int) (polarTop + polarHeight - ScreenUtils.dpToPx(2)));
        ...
        int outFrameBottom = (int) (visibleBottom - ScreenUtils.dpToPx(2));
        ...
        //底部的字显示的位置Y
       /* float y = mDisplayHeight - mTipPaint.getFontMetrics().bottom - tipMarginHeight;
        String time = TimeUtils.dateConvert(System.currentTimeMillis(), TimeUtils.FORMAT_TIME);
    ...
    boolean next() {
        ...
        if (!hasNextChapter()) {
            ToastUtils.getInstance().show(mContext,"您已经阅读完了！");
            return false;
        }
    ...
    private void preLoadNextChapter() {
        ...
        mPreLoadDisp = Single.create((Single.OnSubscribe<List<TxtPage>>) s -> {
            try {
                s.onSuccess(loadPageList(nextChapter));
            } catch (Exception e) {
                e.printStackTrace();
            }
        }).compose(RxUtils::toSimpleSingle)
                .subscribe(new SingleSubscriber<List<TxtPage>>() {
                    @Override
...
//                });
     ...
     * @return
     */
    private List<TxtPage> loadPages(BookChapterEntity.Chapter chapter, BufferedReader br) {
        //生成的页面
    ...
    public interface OnPageChangeListener {
         ...
         */
        void requestChapters(List<BookChapterEntity.Chapter> requestChapters);
         ...
         */
        void onCategoryFinish(List<BookChapterEntity.Chapter> chapters);
