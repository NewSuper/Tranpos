...
import com.newsuper.t.markert.service.UploadTask

import com.newsuper.t.markert.utils.KeyConstrant
import com.transpos.sale.thread.BgThreadFactory
...
class UploadService : JobIntentService() {
    ...
    companion object{
        ...
        private const val INTERNAL_TIME = 5000L
    ...
    override fun onHandleWork(intent : Intent) {
        ...
        if(startLoop){
            ...
            mExecutor?.scheduleAtFixedRate(mTask, INIT_DELAY, INTERNAL_TIME,TimeUnit.MILLISECONDS)
        } else if(stopLoop){
            mExecutor?.shutdown()
            mExecutor = null
        }
