...
//        this.AddKruYuncheck();
    ...
    public void AddKruYuncheck(){
        byte[] command =  new byte[]{(byte) 16, (byte) 4, (byte)2};
        this.addArrayToCommand(command);
    }
     ...
     * @param font
     * @param emphasized   加粗
     * @param doubleheight 双倍高
     * @param doublewidth  双倍宽
     * @param underline    下划线
     */
    ...
    public void addBeep() {
        byte[] command = new byte[]{(byte) 27, (byte) 66, (byte) 9, (byte) 5};
        this.addArrayToCommand(command);
    }

    public void addBeep2() {
        byte[] command = new byte[]{(byte) 27, (byte) 66, (byte) 3, (byte) 3};
        this.addArrayToCommand(command);
    }
    ...
    public void addCODE128(String content) {
        byte[] command = new byte[]{(byte) 29, (byte) 107, (byte) 73, (byte) (content.length() + 2)};
        this.addArrayToCommand(command);
        this.addStrToCommand("{B" + content, command[3]);
    }
