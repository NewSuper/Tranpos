...
import com.newsuper.t.consumer.function.selectgoods.adapter2.ShopMiddleGoodsGridAdapter2;
import com.newsuper.t.consumer.function.selectgoods.adapter2.MiddleGoodsGroupAdapter2;
...
public class SelectMiddleGoodsFragment2 extends Fragment implements ISelectGoodsFragmentView, ISelectGoodsByTypeView, TypeAdapter2.OnItemSelectedListener, IMiddleGoodsShopCart, IGoodsToDetailPage, IShopCart,IShopCartDialog, AnimationListener,
...
//    private View view_line;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        keyWord = ((SelectGoodsActivity3) getActivity()).getKeyWord();
        goods_id = ((SelectGoodsActivity3) getActivity()).getGoods_id();
        searchGoodsItem = ((SelectGoodsActivity3) getActivity()).getSearchGoodsItem();
...
//        view_line.setVisibility(View.GONE);
           ...
           /* //处理店铺收藏
            ...
            if (isFromCollectToLogin) {
                ...
                if ("1".equals(bean.data.is_guodu)) {
                    ...
                    goodsGroupAdapter = new MiddleGoodsGroupAdapter2(getContext(), goodsList, shopCart, shopInfo);
        ...
        if (bean != null) {
            if (null != bean.data.foodlist) {
                if (bean.data.foodlist.size() > 0) {
                    ...
                    if (null == adapter) {
                        adapter = new ShopMiddleGoodsGridAdapter2(getContext(), goodsItems, shopCart, shopInfo);
    ...
    public void showNatureDialog(final GoodsListBean.GoodsInfo goods, final int position) {
        ...
        for (GoodsListBean.GoodsNature goodsNature : goods.nature) {
            goodsNature.data.get(0).is_selected = true;
        }
    ...
    public void deleteNatureGoods(GoodsListBean.GoodsInfo goods, int position, int refreshPos) {
        ...
        if (shopCart.subShoppingSingle(natureGoodsList.get(position))) {
            ...
            natureGoodsAdapter.updateTotalPrice();
            if (isGuodu) {
                goodsGroupAdapter.notifyItemView(goodsPos,refreshPos);
            } else {
    ...
    @Override
    public void showNatureDialog(final GoodsListBean.GoodsInfo goods, final int position,final int typePos) {
        //创建数据集合
        natureGoodsList.clear();
        this.goodsPos=position;
        //用popwindow来显示
        View natureView = View.inflate(getContext(), R.layout.dialog_edit_goods, null);
        TextView goodsName = (TextView) natureView.findViewById(R.id.goods_name);
        tvTotalPrice = (TextView) natureView.findViewById(R.id.tv_price);
        ...
        natureGoodsAdapter = new ShopNatureGoodsAdapter(getContext(), natureGoodsList, shopCart, tvTotalPrice);
        natureGoodsAdapter.setGoodsPosition(typePos);
        rv_nature_container.setAdapter(natureGoodsAdapter);
        natureGoodsAdapter.setGoodsPosition(position);
        natureGoodsAdapter.setDeletNatureListener(this);
        rv_nature_container.setLayoutManager(new LinearLayoutManager(getContext()));
        ...
        ll_add.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                goodsGroupAdapter.notifyItemView(position,typePos);

            }
        });
        LogUtil.log("showNatureDialog","isGuodu == "+isGuodu);
        ll_minus.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                if (natureGoodsList.size() == 0) {
    ...
    private void showCartData() {
        ...
        if (shopCart != null && shopCart.getShoppingAccount() > 0) {
            ...
            if ("1".equals(data.open_promotion)) {
                if (data.promotion_rule != null && data.promotion_rule.size() > 0) {
                    String manjian = "";
