...
public class TagFlowLayout extends FlowLayout implements TagAdapter.OnDataChangedListener {
    ...
    private OnIsCanSelectListener mIsCanSelectListener;
    private boolean isLeastOne=true;//至少选中一个属性
    ...
    public void setIsLeastOne(boolean isLeastOne){
    ...
    //设置一个tag是否可以选中的监听
    public interface OnIsCanSelectListener{
        boolean isCanSelect(int position);
    }
    public void setOnIsCanSelectListener(OnIsCanSelectListener isCanSelectListener) {
        mIsCanSelectListener = isCanSelectListener;
    }
    ...
    private void changeAdapter() {
        ...
        for (int i = 0; i < adapter.getCount(); i++) {
            ...
            if(tagView instanceof TextView){
                if("+自定义".equals(((TextView) tagView).getText().toString())){
                    //屏蔽点击时，选中
                    tagViewContainer.setNoCheckable(true);
                }
            }
    ...
    public void setSelected(int pos){
        TagView tag=(TagView) getChildAt(pos);
        tag.setChecked(true);
    ...
    private void doSelect(TagView child, int position) {
        ...
        if (mAutoSelectEffect) {
                //处理max_select=1的情况
                if (mSelectedMax == 1) {
                    if(null!=mIsCanSelectListener){
                        if(mIsCanSelectListener.isCanSelect(position)){
...
//                            UIUtils.showToast("商品暂时没有库存");
                    ...
                    if (!child.isChecked()){
                        if(mSelectedView.size()<mSelectedMax){
                            ...
                            UIUtils.showToast("至多只能选择"+mSelectedMax+"个属性");
                            return;
                        }
                    }else {
                        if(isLeastOne){
                            if(mSelectedView.size()==1){
                                UIUtils.showToast("至少选择一个属性");
                                return;
                            }
                        }
