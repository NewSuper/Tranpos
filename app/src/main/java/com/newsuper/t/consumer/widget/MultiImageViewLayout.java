...
public class MultiImageViewLayout extends LinearLayout {
    ...
    private int pxImagePadding = dip2px(getContext(), 4);// 图片间的间距
    ...
    public void setList(List<String> lists) throws IllegalArgumentException {
        if (lists == null) {
            throw new IllegalArgumentException("传的图片数组为空...");
        }
        imagesList = lists;
...
//                        ((ImageView) v).setColorFilter(Color.GRAY, PorterDuff.Mode.MULTIPLY);
        ...
        mImageView.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                if (mOnItemClickListener != null) {
                    mOnItemClickListener.onItemClick(v, (Integer) v.getTag(R.id.Comment_List_Position),PressX[0],PressY[0]);
                }
            }
        });
        mImageView.setOnLongClickListener(new OnLongClickListener() {
            ...
            public boolean onLongClick(View v) {
                if (mOnItemClickListener != null) {
                    mOnItemClickListener.onItemLongClick(v, (Integer) v.getTag(R.id.Comment_List_Position),PressX[0],PressY[0]);
                }
                return false;
            }
        });
        if(!TextUtils.isEmpty(url)){
            ...
            mImageView.setTag(R.id.Comment_List_Position, position);
...
//        ImageLoader.getInstance().displayImage(url, mImageView);
    ...
    private void setImageGlide(Context mContext, String PicURL, ImageView mImageView) {
       ...
       UIUtils.glideAppLoad(mContext,PicURL,mImageView);
    }
}