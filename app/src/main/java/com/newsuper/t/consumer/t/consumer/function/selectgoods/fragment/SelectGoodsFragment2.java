package com.newsuper.t.consumer.t.consumer.function.selectgoods.fragment;
...
import com.newsuper.t.consumer.bean.GoodsListBean.GoodsListData;
...
import com.newsuper.t.consumer.function.selectgoods.activity.CouponGoodsActivity;
...
import com.newsuper.t.consumer.function.selectgoods.adapter2.ShopSmallGoodsAdapter;
import com.newsuper.t.consumer.function.selectgoods.adapter.MyItemAnimator;
import com.newsuper.t.consumer.function.selectgoods.adapter.TypeAdapter;
...
import com.newsuper.t.consumer.function.selectgoods.adapter2.ShopGoodsGroupAdapter;
...
import com.newsuper.t.consumer.function.selectgoods.inter.IToCouponGoods;
...
import com.newsuper.t.consumer.widget.ShopCartView;
...
//
public class SelectGoodsFragment2 extends Fragment implements ISelectGoodsFragmentView, ISelectGoodsByTypeView, TypeAdapter.OnItemSelectedListener, IShopCart, IGoodsToDetailPage, IShopCartDialog, AnimationListener,
        View.OnClickListener, IShowLimitTime, ISelectGoodsActivityView, IDeleteNatureGoods, IToCouponGoods {
    ...
    @BindView(R.id.scl_advert)
    NestedScrollView scl_advert;
    @BindView(R.id.iv_advert)
    RoundedImageView iv_advert;
...
//    private ArrayList<GoodsListBean.GoodsInfo> selectGoods;//已选购的商品
    ...
    private ShopGoodsGroupAdapter goodsGroupAdapter;
    private ShopSmallGoodsAdapter adapter;
...
//    private View view_line;
    ...
    private final int COUPON_LOGIN = 13;//点击优惠券跳转登陆的
    ...
    private String keyWord,goods_id;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (activity instanceof SelectGoodsActivity3) {
            shopInfo = ((SelectGoodsActivity3) activity).getShopInfo();
            searchGoodsItem = ((SelectGoodsActivity3) activity).getSearchGoodsItem();
            keyWord = ((SelectGoodsActivity3) activity).getKeyWord();
            goods_id = ((SelectGoodsActivity3) activity).getGoods_id();
            ...
            ShopInfoBean bean = ((TopActivity3) activity).getShopInfoBean();
...
//        load();
    ...
    ArrayList<GoodsListBean.GoodsInfo> cartList;
...
//        getContext().sendBroadcast(intent);
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        rootView = inflater.inflate(R.layout.fragment_select_goods, null);
        ...
        getActivity().registerReceiver(cartReceiver, filter);
        return rootView;
    }
    ...
    private void initView() {
        String url = ((SelectGoodsActivity3)getActivity()).advertising_url;
        if (!StringUtils.isEmpty(url)){
            scl_advert.setVisibility(View.VISIBLE);
            if (!url.startsWith("http")){
                url = RetrofitManager.BASE_IMG_URL + url;
            }
//            Picasso.with(getContext()).load(url).error(R.mipmap.store_logo_default).into(iv_advert);
            UIUtils.glideAppLoad(getContext(),url,R.mipmap.store_logo_default,iv_advert);
        }
...
//        rvGoods.addItemDecoration(typeDivider);
        ...
        llDiscountTip.setOnClickListener(this);
...
//        view_line.setVisibility(View.GONE);
        ...
        LogUtil.log("SelectGoodsFragment2","showLoadingView");
    ...
    private void closeLoadingView() {
        ...
        LogUtil.log("SelectGoodsFragment2","closeLoadingView");
...
//        view_line.setVisibility(View.VISIBLE);
    ...
    private void showShopTipDialog() {
        ...
        View dialogView = LayoutInflater.from(getContext()).inflate(R.layout.dialog_shop_tip, null);
        final Dialog shopTipDialog = new Dialog(getContext(), R.style.CenterDialogTheme2);
        //去掉dialog上面的横线
    ...
    private void initHeadView() {
        ...
        if (null != headType) {
            if (headType.type_id.equals("huodong")) {
                ...
                if ("discount".equals(headType.type_id)) {
                    llDiscountTip.setVisibility(View.VISIBLE);
                } else {
                    llDiscountTip.setVisibility(View.GONE);
                }
    ...
    public void showDataToVIew(GoodsListBean bean) {
        LogUtil.log("SelectGoodsFragment2","showDataToVIew");
        ...
        if (null != bean) {
            ...
            //如果该店铺没有商品
            if (bean.data.foodlist.size() == 0 && bean.data.foodDiscountlist.size() == 0) {
            ...
            if (isFromCollectToLogin) {
                ...
                LogUtil.log("searchGoodsType","4.1、过渡滑动 is_guodu== "+bean.data.is_guodu+"; is_total== "+bean.data.is_total);
                packageItems.clear();
                if ("1".equals(bean.data.is_guodu)) {
                    ...
                    //关闭动画
//                    closeLoadingView();
                    typeAdapter = new TypeAdapter(getContext(), goodsList);
                    typeAdapter.addItemSelectedListener(this);
                    rvClassify.setAdapter(typeAdapter);
                    goodsGroupAdapter = new ShopGoodsGroupAdapter(getContext(), goodsList, shopCart, shopInfo);
                    ...
                    goodsGroupAdapter.setIToCouponGoods(this);
                    initHeadView();
                    //设置过渡滑动的监听
                                ...
                                if (underView != null && underView.getContentDescription() != null) {
                                    ...
                                    if (leftClickType || !goodsType.name.equals(headType.name)) {
                                        if (dy > 0 && headerLayout.getTranslationY() <= 1 && headerLayout.getTranslationY() >= -1 * headerLayout.getMeasuredHeight() * 4 / 5 && !leftClickType) {
                                            ...
                                            if (goodsType.type_id.equals("huodong")) {
                                                headerLayout.setVisibility(View.INVISIBLE);
                                            } else {
                                                ...
                                                if ("discount".equals(goodsType.type_id)) {
                                                    llDiscountTip.setVisibility(View.VISIBLE);
                                                } else {
                                                    llDiscountTip.setVisibility(View.GONE);
                                                }
                                            ...
                                            headType = goodsType;
                                            if ("huodong".equals(headType.type_id)) {
                                                headerLayout.setVisibility(View.INVISIBLE);
                                            } else {
                                                headerLayout.setVisibility(View.VISIBLE);
                                                if ("search".equals(headType.type_id)) {
                    ...
                    /*for (GoodsType goodsType : goodsList) {
                        ...
                        while (iterator.hasNext()) {
                            ...
                            GoodsListBean.GoodsInfo goodsInfo = shopCart.getGoodsMap().get(id).get(0);
                            if (goodsType.type_id.equals(goodsInfo.type_id)) {
                                goodsType.count += shopCart.getGoodsCount(goodsInfo.id);
                            }
                        }
                    }*/
                    refreshGoodsTypeCount();
...
//            if ("huodong".equals(headType.type_id)) {
            goodsType = goodsGroupAdapter.getGoodsTypeByPosition(position);
//            } else {
//                goodsType = goodsGroupAdapter.getGoodsTypeByPosition(position + 1);
//            }
            headType = goodsType;
            if ("huodong".equals(headType.type_id)) {
                headerLayout.setVisibility(View.INVISIBLE);
        ...
        if (bean != null) {
            if (null != bean.data.foodlist) {
                if (bean.data.foodlist.size() > 0) {
                    ...
                    if (null == adapter) {
                        adapter = new ShopSmallGoodsAdapter(getContext(), goodsItems, shopCart, shopInfo);
                        if (rvGoods != null) {
                        ...
                        adapter.setIToCouponGoods(this);
                    } else {
    ...
    private void showCartData() {
        ...
        if (shopCart != null && shopCart.getShoppingAccount() > 0) {
            ...
            if (shopCart.getShoppingTotalPrice() < Float.parseFloat(basicPrice)) {
                ...
                tvTips.setText(FormatUtil.numFormat(elsePrice) + "元起送");
            } else {
    ...
    private void showDiscountTip() {
        View dialogView = View.inflate(getContext(), R.layout.dialog_discount_tip, null);
        final Dialog discountDialog = new Dialog(getContext(), R.style.CenterDialogTheme2);
        //去掉dialog上面的横线
        Context context = discountDialog.getContext();
        int divierId = context.getResources().getIdentifier("android:id/titleDivider", null, null);
        View divider = discountDialog.findViewById(divierId);
        if (null != divider) {
            divider.setBackgroundColor(Color.TRANSPARENT);
        }
        discountDialog.setContentView(dialogView);
    ...
    private void showCartDetail() {
        if (shopCart != null && shopCart.getShoppingAccount() > 0) {
         ...
         /*  if (!shopCartView.isBindView()){
               shopCartView.bindView(shopCart,shop_id,isEffectiveVip);
               shopCartView.setIShopCartDialog(this);
           }
           shopCartView.showCartView();*/
        }
    }
    ...
    public void updateCartData() {
        showCartData();
        LogUtil.log("updateCartData","  刷新商品分类 ");
        //刷新商品分类列表
        refreshGoodsTypeCount();
    ...
    public void onDestroyView() {
        ...
        if (null != cartReceiver) {
            if (getActivity() != null){
                getActivity().unregisterReceiver(cartReceiver);
            }
        }
    }
    ...
    @Override
    public void toCouponGoods() {
        //默认活动商品页面
        Intent intent = new Intent(getContext(), CouponGoodsActivity.class);
        intent.putExtra("shop_id", shop_id);
        intent.putExtra("shopInfo", shopInfo);
        intent.putExtra("basicPrice", basicPrice);
        intent.putExtra("shop_cart", shopCart);
        startActivity(intent);
    }
    ...
    public void showShareDilaog() {
        if (shareDialog == null) {
            ...
            ((LinearLayout) view.findViewById(R.id.ll_share_friend)).setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    new ShareUtils().WXShareUrl(msgApi, RetrofitManager.BASE_URL_H5 + "h5/lwm/share/shareshop?admin_id=" + admin_id + "&shop_id=" + shop_id + "&bundleName="+getString(R.string.share_scheme) + "&customerapp_id=" + RetrofitUtil.ADMIN_APP_ID, picUrl,
            ...
            ((LinearLayout) view.findViewById(R.id.ll_share_qq)).setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    ShareUtils.QQshare(tencent, getActivity(), shopInfo.shopname, RetrofitManager.BASE_URL_H5 + "h5/lwm/share/shareshop?admin_id=" + admin_id + "&shop_id=" + shop_id + "&bundleName="+getString(R.string.share_scheme) + "&customerapp_id=" + RetrofitUtil.ADMIN_APP_ID,
                            des, picUrl);
                    shareDialog.dismiss();
                }
            });
...
//        Set<String> sets = null;
        for (GoodsListBean.GoodsInfo goodsInfo:list){
            ...
            LogUtil.log("isFullMinBuy","goodsInfo.name == "+ goodsInfo.name + " min_buy == "+ min +" count == "+map.get(goodsInfo.id));
...
//                LogUtil.log("isFullMinBuy","goodsInfo.type ==  has");
//                if (sets == null){
//                    sets = new HashSet<>();
//                }
//                if (typeAdapter.getTypeNeedMap().containsKey(goodsInfo.type_id)){
//                    LogUtil.log("isFullMinBuy","goodsInfo.typeid =="+goodsInfo.type_id);
...
//            LogUtil.log("isFullMinBuy","goodsInfo.type  0000 ==");
...
//                LogUtil.log("updateCartData","goods == "+goodsInfo.name +" discount == "+data.foodDiscountlist.size());
//                //判断是否为折扣商品
//                for (GoodsListBean.GoodsInfo goods : data.foodDiscountlist) {
...
//                    if (goodsInfo.type_id.equals(type.type_id) || isDiscountGoods) {
    ...
    private void refreshGoodsTypeCount(){
        for (GoodsType type : goodsList) {
            if ("search".equals(type.type_id))
                continue;
            int count = 0;
            LogUtil.log("updateCartData","type.name == "+type.name + "; type_id== " + type.type_id + " start ---------------");
            Iterator<String> iterator = shopCart.getGoodsMap().keySet().iterator();
            while (iterator.hasNext()) {
                String id = iterator.next();
                GoodsListBean.GoodsInfo goodsInfo = shopCart.getGoods(id);
