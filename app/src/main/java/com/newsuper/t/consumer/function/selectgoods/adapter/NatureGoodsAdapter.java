...
public class NatureGoodsAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public NatureGoodsAdapter(Context context, ArrayList<GoodsListBean.GoodsInfo> data, ShopCart shopCart, TextView tvTotalPrice) {
                    ...
                    TagAdapter<GoodsListBean.PackageNatureValue> valueAdapter = new TagAdapter<GoodsListBean.PackageNatureValue>(packageNature.value) {
                        ...
                        public View getView(FlowLayout parent, int position, GoodsListBean.PackageNatureValue packageNatureValue) {
                            ...
                            if ("1".equals(packageNatureValue.stockvalid)) {
                                ...
                                if (shopCart.goodsSingle.containsKey(packageNatureValue.id) && shopCart.goodsSingle.get(packageNatureValue.id) == packageNatureValue.stock) {
                                ...
                                if (selectPosSet.contains(i)) {
                                    if (!TextUtils.isEmpty(goods.index)) {
                                        if (!packageNature.value.get(i).is_selected) {
                                            if (shopCart.goodsSingle.containsKey(packageNature.value.get(i).id)) {
                                                int num = shopCart.goodsSingle.get(packageNature.value.get(i).id);
                                                num++;
                                                shopCart.goodsSingle.put(packageNature.value.get(i).id, num);
                                            } else {
                                                shopCart.goodsSingle.put(packageNature.value.get(i).id, 1);
                                            }
                                        }
                                    }
                                    ...
                                    if (!TextUtils.isEmpty(goods.index)) {
                                        if (packageNature.value.get(i).is_selected) {
                                            if (shopCart.goodsSingle.containsKey(packageNature.value.get(i).id)) {
                                                ...
                                                num--;
                                                if (num == 0) {
                                                    shopCart.goodsSingle.remove(packageNature.value.get(i).id);
                                                } else {
                                                    shopCart.goodsSingle.put(packageNature.value.get(i).id, num);
                                                }
                    ...
                    valueFlowLayoutvalue.setOnIsCanSelectListener(new TagFlowLayout.OnIsCanSelectListener() {
                        ...
                        public boolean isCanSelect(int position) {
                            ...
                            if ("1".equals(selectValue.stockvalid)) {
                                ...
                                if (shopCart.goodsSingle.containsKey(selectValue.id) && shopCart.goodsSingle.get(selectValue.id) == selectValue.stock) {
