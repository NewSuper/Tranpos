...
import com.xunjoy.lewaimai.consumer.function.selectgoods.adapter.GoodsSearchAdapter;
...
public class ShopGoodsSearchAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public ShopGoodsSearchAdapter(Context mContext, ArrayList<GoodsListBean.GoodsInfo> data, ShopCart2 shopCart, ShopInfoBean.ShopInfo shopInfo) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (viewType == TYPE_FOOTER) {
            ...
            View viewGoods = LayoutInflater.from(mContext).inflate(R.layout.item_search_goods, parent, false);
    ...
    public class GoodsViewHolder extends RecyclerView.ViewHolder {
        public TextView name, tv_fuhao, price, tvCount, tvShowCount, tvQi, tv_huodong, tv_desc, tvSaleCount, tvUnit, tv_old_price,
                tvVipPrice, tvLimitTag,tvVipQi,tv_discount_num,tv_discount,tv_min_buy;
        public ImageView img, iv_sale_out;
        public LinearLayout goods_layout, llAdd, ll_edit_goods, llMinus, ll_show_vip, ll_root, ll_limit_time,ll_discount;
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, final int position, List<Object> payloads) {
        LogUtil.log("ShopppingCartAction",position +" onBindViewHolder   isEmpty" + payloads.isEmpty());
        if (payloads.isEmpty()) {
            onBindViewHolder(holder, position);
        } else {
            GoodsViewHolder goodsHolder = (GoodsViewHolder) holder;
            final GoodsListBean.GoodsInfo goods = data.get(position);
            LogUtil.log("ShopppingCartAction",position +" onBindViewHolder  00000000" + goods.name);
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, final int position) {
        ...
        if (goodsHolder != null) {
            ...
            //加载网络图片
            UIUtils.glideAppLoad(mContext,goods.img,R.mipmap.common_def_food,goodsHolder.img);
            //商品标签
            ...
            if (null!=goods.packageNature&&goods.packageNature.size()>0) {
                if (1!=shopInfo.worktime && StringUtils.isEmpty(goods.outtime_info)) {
            ...
            if ("1".equals(goods.is_limitfood)) {
                ...
                goodsHolder.tvLimitTag.setVisibility(View.GONE);
            }
                                        ...
                                        goodsHolder.llAdd.setOnClickListener(new View.OnClickListener() {
                                            ...
                                            public void onClick(View view) {
                                                ...
                                                if (shopCart.addShoppingSingle(goods)) {
                                                    LogUtil.log("ShopppingCartAction",position +" addShoppingSingle   isEmpty" + goods.name);
