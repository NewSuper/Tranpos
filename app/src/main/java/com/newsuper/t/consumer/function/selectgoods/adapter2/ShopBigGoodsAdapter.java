...
import com.xunjoy.lewaimai.consumer.function.selectgoods.adapter.BigGoodsAdapter;
...
public class ShopBigGoodsAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public ShopBigGoodsAdapter(Context mContext, ArrayList<GoodsListBean.GoodsInfo> data, ShopCart2 shopCart, ShopInfoBean.ShopInfo shopInfo) {
    ...
    public int setSelectGoodsId(String goodsId){
        LogUtil.log("SelectSearchGoods","setSelectGoodsId   id = "+goodsId);
        if (StringUtils.isEmpty(goodsId) && data.size() ==0){
            return 0;
        }
        this.selectGoodsId = goodsId;
        for (int i = 0; i < data.size() ; i ++){
            if (goodsId.equals(data.get(i).id)){
