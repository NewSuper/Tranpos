...
public class ShopCartDetailAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public ShopCartDetailAdapter(Context mContext, ShopCart2 shopCart, boolean isEffectiveVip, String shop_id) {
        ...
        if (payloads.isEmpty()) {
            ...
            if ("1".equals(goods.switch_discount)) {
                ...
                if ("1".equals(goods.member_price_used)) {
                    if (isEffectiveVip) {
                        formerprice = MemberUtil.getMemberPriceFloat(goods.member_grade_price);
                    } else {
            ...
            if (null != goods.nature && goods.nature.size() > 0) {
                ...
                if ("1".equals(goods.switch_discount)) {
                    //判断选购的折扣商品中是否含有原价
                    int formerNum = 0;
                    for (GoodsListBean.GoodsInfo cartGoods : shopCart.getGoodsList(goods.id)) {
                ...
                //普通商品的处理
                count = shopCart.getGoodsCount(goods.id);
                if ("1".equals(goods.switch_discount)) {
